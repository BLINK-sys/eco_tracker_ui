...
import { updateLocation, createContainer, deleteContainer } from "@/services/dataService";
...
interface EditLocationDialogProps {
  location: Location;
  onUpdateLocation: (updatedLocation: { id: string; name: string; lat: number; lng: number; containerCount: number; address: string }) => void;
  children: React.ReactNode;
}

const EditLocationDialog: React.FC<EditLocationDialogProps> = ({ location, onUpdateLocation, children }) => {
  ...
  const [containerCount, setContainerCount] = useState(location.containers.length);
  const [isLoading, setIsLoading] = useState(false);
  const { refreshLocations } = useAuth();
  ...
  useEffect(() => {
    if (location.address && !location.address.startsWith("Координаты:")) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Обновляем основную информацию о площадке
      ...
      // Управление контейнерами - если количество изменилось
      const currentContainerCount = location.containers.length;
      
      if (containerCount > currentContainerCount) {
        // Добавляем новые контейнеры
        const newContainersCount = containerCount - currentContainerCount;
        for (let i = 0; i < newContainersCount; i++) {
          await createContainer({
            location_id: location.id,
            number: currentContainerCount + i + 1,
            status: 'empty',
            fill_level: 0
          });
        }
        toast({
          title: "Контейнеры добавлены",
          description: `Добавлено ${newContainersCount} новых контейнеров.`,
        });
      } else if (containerCount < currentContainerCount) {
        // Удаляем лишние контейнеры (начиная с конца)
        const containersToRemove = location.containers.slice(containerCount);
        for (const container of containersToRemove) {
          await deleteContainer(container.id);
        }
        toast({
          title: "Контейнеры удалены",
          description: `Удалено ${containersToRemove.length} контейнеров.`,
        });
      }

      // Обновляем локальное состояние (для обратной совместимости)
      onUpdateLocation({
        id: location.id,
        name: name.trim(),
        lat: selectedPosition.lat,
      ...
      // Обновляем список площадок из контекста
      await refreshLocations();

      toast({
        title: "Площадка обновлена",
        description: `Площадка "${name}" успешно обновлена.`,
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Не удалось обновить площадку",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
